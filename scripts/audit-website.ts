...
async function checkVisualConsistency(page: Page): Promise<VisualIssue[]> {
  ...
  visualIssues.push(...issues.map(issue => ({
    ...issue,
...
async function checkNavigation(page: Page, browser: Browser): Promise<NavigationIssue[]> {
  ...
  const menuToggleExists = await page.evaluate(() => {
    const toggles = document.querySelectorAll('[class*="menu-toggle"], [class*="hamburger"], button[aria-label*="menu" i]');
  ...
  const hasBreadcrumbs = await page.evaluate(() => {
    const breadcrumbs = document.querySelector('[aria-label*="breadcrumb" i], .breadcrumb, nav[class*="breadcrumb"]');
...
async function checkSEO(page: Page): Promise<SEOIssue[]> {
  ...
  const seoData = await page.evaluate(() => {
    const data: Record<string, unknown> = {
      ...
      // Structured data
      hasStructuredData: Array.from(document.querySelectorAll('script[type="application/ld+json"]')).length > 0,
      ...
      // Images
      imagesWithoutAlt: Array.from(document.querySelectorAll('img:not([alt]), img[alt=""]')).length,
      totalImages: document.querySelectorAll('img').length,
      
      // Links
      internalLinks: Array.from(document.querySelectorAll('a[href^="/"], a[href^="' + window.location.origin + '"]')).length,
      externalLinks: Array.from(document.querySelectorAll('a[href^="http"]')).filter(
  ...
  if (!seoData.title || typeof seoData.title !== 'string') {
    ...
    seoIssues.push({
      ...
      recommendation: 'Keep page title under 60 characters for optimal display in search results'
    });
    ...
    seoIssues.push({
      ...
      recommendation: 'Consider a more descriptive title (aim for 50-60 characters)'
    });
  }
  ...
  } else if (typeof seoData.metaDescription === 'string' && seoData.metaDescription.length > 160) {
    seoIssues.push({
  ...
  if (typeof seoData.h1Count === 'number' && seoData.h1Count === 0) {
    seoIssues.push({
      ...
      recommendation: 'Add exactly one H1 heading that describes the main content'
    });
  ...
  if (!seoData.hasStructuredData) {
    seoIssues.push({
      ...
      recommendation: 'Add structured data markup for better search engine understanding'
    });
  }
  ...
  // Check images
  if (typeof seoData.imagesWithoutAlt === 'number' && typeof seoData.totalImages === 'number' && seoData.imagesWithoutAlt > 0) {
    seoIssues.push({
  ...
  if (typeof seoData.wordCount === 'number' && seoData.wordCount < 300) {
    seoIssues.push({
      ...
      recommendation: 'Consider adding more substantial content (aim for 300+ words)'
    });
  }
  ...
  if (typeof seoData.internalLinks === 'number' && seoData.internalLinks < 3) {
    seoIssues.push({
      ...
      recommendation: 'Add more internal links to improve site structure and navigation'
    });
  }
...
async function runAudit() {
  ...
  try {
    ...
    // Generate summary
    const totalIssues = brokenLinks.length + visualIssues.length + 
                       navigationIssues.length + seoIssues.length;
    
    const summary = `This audit identified ${totalIssues} total issues across the website:\n\n` +
